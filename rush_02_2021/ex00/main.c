...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/08/21 19:05:42 by sgaladri          #+#    #+#             */
/*   Updated: 2021/08/22 17:46:38 by lliane           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parsefirst(char *searchstr, int length, char *dictname, int *f)
{
	...
	else if (length % 3 == 2)
	{
		...
		else if ((*searchstr > '1') && (*f != 2))
			res = outth_segment(searchstr, dictname);
	}
	else if ((*f != 2) && (*searchstr != '0'))
	{
		res = outf_segment(searchstr, dictname);
		*f = 1;
	}
...
int	convertnum(char *str, char *dictname)
{
	...
	if (*str == '0')
		return (ft_conversion("0", dictname));
	flag = 0;
	res = 0;
	while (*str)
	{
...
int	main(int argc, char **argv)
{
	...
	if (((argc < 2) || (argc > 3)))
	{
		ft_initial(error, argc, argv[1]);
		error = 1;
	}
	else if (argc == 2)
	{
		error = convertnum(argv[1], "./dictionaries/numbers.dict");
		if (error)
			outerror(error);
	}
	else if (argc == 3)
	{
		error = convertnum(argv[2], argv[1]);
		if (error)
			outerror(error);
	}
	if (!error)
